...
static int pci_quirk_rockchip_rp_acs(struct pci_dev *dev, u16 acs_flags)
{
	/*
	 * RK3588 DWC Root Port does not expose an ACS capability in hardware.
	 * The ARM SMMUv3 provides equivalent isolation: each PCIe function has
	 * an independent Stream ID backed by its own page table, so peer-to-peer
	 * DMA between functions is impossible at the SMMU level.
	 */
	return pci_acs_ctrl_enabled(acs_flags,
		PCI_ACS_SV | PCI_ACS_RR | PCI_ACS_CR | PCI_ACS_UF);
}
...
static const struct pci_dev_acs_enabled {
	...
	/* Rockchip RK3588 Root Port (no ACS capability exposed, SMMUv3 provides isolation) */
	{ PCI_VENDOR_ID_ROCKCHIP, 0x3588, pci_quirk_rockchip_rp_acs },
