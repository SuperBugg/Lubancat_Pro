...
int vfio_platform_irq_init(struct vfio_platform_device *vdev)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		unsigned long irqflags = vdev->irq_flags;
		...
		if (!(vdev->irq_flags & IRQF_SHARED) &&
		    (irq_get_trigger_type(hwirq) & IRQ_TYPE_LEVEL_MASK)) {
		...
		dev_info(vdev->device,
			 "CHJ vfio-platform: request irq %d flags=0x%lx\n",
			 hwirq, irqflags);
		ret = request_irq(hwirq, handler, irqflags,
				  vdev->irqs[i].name, &vdev->irqs[i]);
		if (ret) {
			dev_info(vdev->device,
				 "CHJ vfio-platform: request irq %d ret=%d\n",
				 hwirq, ret);
