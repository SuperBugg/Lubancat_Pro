...
static vfio_platform_reset_fn_t vfio_platform_lookup_reset(const char *compat,
					...
					vfio_platform_release_fn_t *release,
					unsigned long *irq_flags)
{
	...
	list_for_each_entry(iter, &reset_list, link) {
		if (!strcmp(iter->compat, compat) &&
			...
			*release = iter->of_release;
			*irq_flags = iter->irq_flags;
			break;
		}
	}
...
static int vfio_platform_get_reset(struct vfio_platform_device *vdev)
{
	...
	if (vdev->of_reset && vdev->irq_flags)
		dev_info(vdev->device,
			 "CHJ vfio-platform: reset quirk irq_flags=0x%lx\n",
			 vdev->irq_flags);
...
int vfio_platform_open_device(struct vfio_device *core_vdev)
{
	...
	dev_info(vdev->device, "CHJ vfio-platform: open enter\n");

	dev_info(vdev->device, "CHJ vfio-platform: regions_init enter\n");
	ret = vfio_platform_regions_init(vdev);
	if (ret) {
		dev_info(vdev->device,
			 "CHJ vfio-platform: regions_init ret=%d\n", ret);
		return ret;
	}
	dev_info(vdev->device, "CHJ vfio-platform: regions_init ok\n");

	dev_info(vdev->device, "CHJ vfio-platform: irq_init enter\n");
	ret = vfio_platform_irq_init(vdev);
	if (ret) {
		dev_info(vdev->device,
			 "CHJ vfio-platform: irq_init ret=%d\n", ret);
		goto err_irq;
	}
	dev_info(vdev->device, "CHJ vfio-platform: irq_init ok\n");

	dev_info(vdev->device, "CHJ vfio-platform: pm_runtime_get_sync enter\n");
	ret = pm_runtime_get_sync(vdev->device);
	dev_info(vdev->device,
		 "CHJ vfio-platform: pm_runtime_get_sync ret=%d\n", ret);
	if (ret < 0)
		goto err_rst;

	dev_info(vdev->device, "CHJ vfio-platform: reset enter\n");
	ret = vfio_platform_call_reset(vdev, &extra_dbg);
	dev_info(vdev->device, "CHJ vfio-platform: reset ret=%d %s\n", ret,
		 extra_dbg ? extra_dbg : "");
	...
	dev_info(vdev->device, "CHJ vfio-platform: open ok\n");
	return 0;

err_rst:
	dev_info(vdev->device, "CHJ vfio-platform: err_rst cleanup\n");
	pm_runtime_put(vdev->device);
	vfio_platform_irq_cleanup(vdev);
err_irq:
	dev_info(vdev->device, "CHJ vfio-platform: err_irq cleanup ret=%d\n",
		 ret);
	vfio_platform_regions_cleanup(vdev);
	return ret;
}
