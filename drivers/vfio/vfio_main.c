...
static struct vfio_group *vfio_group_find_or_alloc(struct device *dev)
{
	...
	dev_info(dev, "CHJ vfio_group_find_or_alloc enter\n");
	...
	if (!iommu_group && vfio_noiommu) {
		 ...
		 * capable device to a user without IOMMU protection.
		 */
		dev_info(dev, "CHJ no iommu_group, using vfio-noiommu path\n");
	...
	if (!iommu_group) {
		dev_err(dev, "CHJ fail: iommu_group_get returned NULL\n");
		return ERR_PTR(-EINVAL);
	}

	dev_info(dev, "CHJ iommu_group_get ok: group_id=%d\n",
		 iommu_group_id(iommu_group));

	group = vfio_group_get_from_iommu(iommu_group);
	if (group) {
		dev_info(dev, "CHJ got existing vfio_group from iommu group\n");
	} else {
		dev_info(dev, "CHJ creating new vfio_group\n");
		group = vfio_create_group(iommu_group, VFIO_IOMMU);
	}

	if (IS_ERR(group))
		dev_err(dev, "CHJ fail: vfio group create/get ret=%ld\n",
			PTR_ERR(group));
	else
		dev_info(dev, "CHJ vfio_group_find_or_alloc success\n");
