...
// 	pci_info(pdev,
// 		 "CHJ core_register enter: hdr_type=0x%x is_virtfn=%d is_physfn=%d num_vfs=%d\n",
// 		 pdev->hdr_type, pdev->is_virtfn, pdev->is_physfn, pci_num_vf(pdev));


// 	/* Drivers must set the vfio_pci_core_device to their drvdata */
// 	if (WARN_ON(vdev != dev_get_drvdata(dev)))
// 	{
// 		pci_err(pdev, "CHJ fail: drvdata mismatch\n");
// 		return -EINVAL;
// 	}
...
// 	{
// 		pci_err(pdev, "CHJ fail: non-normal hdr_type=0x%x\n", pdev->hdr_type);
// 		return -EINVAL;
// 	}
...
// 	 */
// 	if (pci_num_vf(pdev))  {
// 		pci_warn(pdev, "CHJ fail: PF has VFs enabled, num_vfs=%d\n",
// 			 pci_num_vf(pdev));
// 		return -EBUSY;
// 	}
...
int vfio_pci_core_register_device(struct vfio_pci_core_device *vdev)
{
	...
	pci_info(pdev,
		 "CHJ core_register enter: vendor=%04x device=%04x class=0x%x hdr_type=0x%x is_virtfn=%d is_physfn=%d num_vfs=%d driver_override=%s\n",
		 pdev->vendor, pdev->device, pdev->class, pdev->hdr_type,
		 pdev->is_virtfn, pdev->is_physfn, pci_num_vf(pdev),
		 pdev->driver_override ? pdev->driver_override : "NULL");
	...
	if (vdev->vdev.mig_ops) {
		if (!(vdev->vdev.mig_ops->migration_get_state &&
		    ...
		    !(vdev->vdev.migration_flags & VFIO_MIGRATION_STOP_COPY)) {
			pci_err(pdev,
				"CHJ fail: invalid migration ops, migration_flags=0x%x\n",
				vdev->vdev.migration_flags);
			return -EINVAL;
		}

		pci_info(pdev, "CHJ migration ops check ok\n");
	}

	if (vdev->vdev.log_ops &&
	    !(vdev->vdev.log_ops->log_start &&
	      vdev->vdev.log_ops->log_stop &&
	      vdev->vdev.log_ops->log_read_and_clear)) {
		pci_err(pdev, "CHJ fail: invalid log ops\n");
		return -EINVAL;
	}

	if (vdev->vdev.log_ops)
		pci_info(pdev, "CHJ log ops check ok\n");
	...
	if (pci_num_vf(pdev)) {
		pci_warn(pdev,
			 "CHJ fail: PF has VFs enabled, num_vfs=%d\n",
			 pci_num_vf(pdev));
		return -EBUSY;
	}

	if (pci_is_root_bus(pdev->bus)) {
		pci_info(pdev, "CHJ assign device set: root bus, set=vdev\n");
		ret = vfio_assign_device_set(&vdev->vdev, vdev);
	} else if (!pci_probe_reset_slot(pdev->slot)) {
		pci_info(pdev, "CHJ assign device set: slot reset supported, set=slot\n");
		ret = vfio_assign_device_set(&vdev->vdev, pdev->slot);
	} else {
		pci_info(pdev, "CHJ assign device set: no slot reset, set=bus\n");
		ret = vfio_assign_device_set(&vdev->vdev, pdev->bus);
	}

	if (ret) {
		pci_err(pdev, "CHJ fail: vfio_assign_device_set ret=%d\n", ret);
		return ret;
	}

	pci_info(pdev, "CHJ vfio_assign_device_set ok\n");

	ret = vfio_pci_vf_init(vdev);
	if (ret) {
		pci_err(pdev,
			"CHJ fail: vfio_pci_vf_init ret=%d is_virtfn=%d physfn=%s\n",
			ret, pdev->is_virtfn,
			pdev->is_virtfn ? pci_name(pdev->physfn) : "none");
		return ret;
	}

	pci_info(pdev, "CHJ vfio_pci_vf_init ok\n");

	ret = vfio_pci_vga_init(vdev);
	if (ret) {
		pci_err(pdev, "CHJ fail: vfio_pci_vga_init ret=%d\n", ret);
		goto out_vf;
	}

	pci_info(pdev, "CHJ vfio_pci_vga_init ok\n");

	vfio_pci_probe_power_state(vdev);
	pci_info(pdev, "CHJ vfio_pci_probe_power_state done\n");
	 ...
	 */
	vfio_pci_set_power_state(vdev, PCI_D0);
	pci_info(pdev, "CHJ set power state D0 done\n");

	dev->driver->pm = &vfio_pci_core_pm_ops;
	pm_runtime_allow(dev);

	if (!disable_idle_d3) {
		pci_info(pdev, "CHJ runtime PM put, disable_idle_d3=0\n");
		pm_runtime_put(dev);
	} else {
		pci_info(pdev, "CHJ skip runtime PM put, disable_idle_d3=1\n");
	}

	ret = vfio_register_group_dev(&vdev->vdev);
	if (ret) {
		pci_err(pdev, "CHJ fail: vfio_register_group_dev ret=%d\n", ret);
		goto out_power;
	}

	pci_info(pdev, "CHJ vfio_register_group_dev ok, probe success\n");
	return 0;
...
out_power:
	...
	pm_runtime_forbid(dev);
	pci_err(pdev, "CHJ out_power ret=%d\n", ret);

out_vf:
	vfio_pci_vf_uninit(vdev);
	pci_err(pdev, "CHJ out_vf ret=%d\n", ret);
