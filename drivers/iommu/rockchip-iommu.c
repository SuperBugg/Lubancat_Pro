...
static bool rk_iommu_is_stall_active(struct rk_iommu *iommu)
{
	bool active = true;
	int i;
	// if(pm_runtime_suspended(iommu->dev)){
	// 	return 
	// }
	for (i = 0; i < iommu->num_mmu; i++)
...
static void rk_iommu_iotlb_sync(struct iommu_domain *domain,
				struct iommu_iotlb_gather *gather)
{
	...
	length = gather->end - gather->start + 1;
	rk_iommu_zap_iova(rk_domain, (dma_addr_t)gather->start, length);
}

static void rk_iommu_iotlb_sync_map(struct iommu_domain *domain,
				   unsigned long iova, size_t size)
{
	return;
}

static bool rk_iommu_enforce_cache_coherency(struct iommu_domain *domain)
{
	return true;
}
			     ...
			     size_t size, struct iommu_iotlb_gather *gather)
{
	...
	if(gather){
		iommu_iotlb_gather_add_range(gather, iova, unmap_size);
	}
	else {
...
// 	/* First force reset to put hardware into a known, initialized state */
				   ...
				   struct device *dev)
{
	...
	if (iommu->dlr_disable) {
		/*
		 * IOMMUs marked with "rockchip,disable-device-link-resume"
		 * (e.g. RK3588 rknpu_mmu) have their register banks inside
		 * the master device's power islands. The master driver is
		 * responsible for the IOMMU enable/disable lifecycle via
		 * rockchip_iommu_enable() / rockchip_iommu_disable(), which
		 * are issued while the islands and clocks are guaranteed
		 * up.
		 *
		 * By the time detach runs (typically during driver unbind),
		 * the master has either already disabled the IOMMU and
		 * gated the islands (rknpu_power_off path), or never
		 * enabled it at all. In either case the iommu->dev runtime
		 * PM state is unreliable here -- device_link keeps it
		 * looking "active" while the underlying power is actually
		 * off -- and touching MMU registers would SError the
		 * kernel.
		 *
		 * Skip the hardware path entirely: there is no state to
		 * tear down, and the next attach will re-enable through
		 * the master's rockchip_iommu_enable() call.
		 */
		iommu->iommu_enabled = false;
		return;
	}
...
static struct iommu_device *rk_iommu_probe_device(struct device *dev)
{
	...
	if (!data) {
		/*
		 * of_iommu_configure() may have run before this IOMMU was
		 * registered. Re-run of_xlate to set up priv data now.
		 */
		ret = of_parse_phandle_with_args(dev->of_node, "iommus",
						 "#iommu-cells", 0, &args);
		if (ret)
			return ERR_PTR(-ENODEV);
		ret = rk_iommu_of_xlate(dev, &args);
		of_node_put(args.np);
		if (ret)
			return ERR_PTR(-ENODEV);
		data = dev_iommu_priv_get(dev);
			     ...
			     struct of_phandle_args *args)
{
	...
	struct rk_iommudata *data;
	struct rk_iommu *iommu;
	struct device *alloc_dev = dma_dev ?: dev;
	...
	data = devm_kzalloc(alloc_dev, sizeof(*data), GFP_KERNEL);
	if (!data) {
		platform_device_put(iommu_dev);
		return -ENOMEM;
	}
...
static bool rk_iommu_capable(struct device *dev, enum iommu_cap cap)
{
	switch (cap) {
	case IOMMU_CAP_CACHE_COHERENCY:
		/*
		 * The page table walker reads PTEs out of cache-coherent
		 * DDR, and all client masters on this IOMMU snoop the CPU
		 * caches.  Mappings with IOMMU_CACHE work correctly.
		 */
		return true;
	default:
		return false;
...
{
	/*
	 * Keep the historical default: DMA-API-managed domain for
	 * regular consumers (rga / vop / vdpu / jpegd / npu via
	 * dma-iommu).  VFIO clients explicitly request UNMANAGED
	 * through iommu_domain_alloc(IOMMU_DOMAIN_UNMANAGED).
	 */
...
static const struct iommu_ops rk_iommu_ops = {
	...
	.default_domain_ops = &(const struct iommu_domain_ops) {
		...
		.iotlb_sync	= rk_iommu_iotlb_sync,
		.iotlb_sync_map	= rk_iommu_iotlb_sync_map,
		.iova_to_phys	= rk_iommu_iova_to_phys,
		.enforce_cache_coherency = rk_iommu_enforce_cache_coherency,
...
alloc_group:
	 ...
	 * IOMMU device. Set before iommu_device_register() so that
	 * bus_iommu_probe()->probe_device()->of_xlate fallback can use it.
	 */
	if (!dma_dev)
		dma_dev = &pdev->dev;

	/*
	 * Optional IOMMU backends (currently AV1D) keep their private state in
	 * dev->platform_data. iommu_device_register() immediately replays
	 * bus_iommu_probe(), which can attach devices and later free domains,
	 * so the optional backend must be fully initialized first.
	 */
