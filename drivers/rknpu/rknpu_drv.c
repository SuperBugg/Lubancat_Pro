...
bool rknpu_is_iommu_enable(struct device *dev)
{
...
static int rknpu_power_on(struct rknpu_device *rknpu_dev)
{
	...
	ret = rknpu_hw_power_on(&rknpu_dev->hw_res);

	if (ret == 0 && rknpu_dev->config->state_init != NULL)
...
static int rknpu_power_off(struct rknpu_device *rknpu_dev)
{
	...
	ret = rknpu_hw_power_off(&rknpu_dev->hw_res);
...
static int rknpu_probe(struct platform_device *pdev)
{
	...
	/*
	 * Populate hardware resources for rknpu_hw_* wrappers.
	 * rknpu_hw_acquire_resources() is the equivalent for the VFIO reset
	 * path (where the main driver is not bound).
	 */
	rknpu_dev->hw_res.dev = dev;
	rknpu_dev->hw_res.iommu_en = rknpu_dev->iommu_en;
	rknpu_dev->hw_res.iommu_group = rknpu_dev->iommu_group;
	rknpu_dev->hw_res.clks = rknpu_dev->clks;
	rknpu_dev->hw_res.num_clks = rknpu_dev->num_clks;
	rknpu_dev->hw_res.genpd_dev_npu0 = rknpu_dev->genpd_dev_npu0;
	rknpu_dev->hw_res.genpd_dev_npu1 = rknpu_dev->genpd_dev_npu1;
	rknpu_dev->hw_res.genpd_dev_npu2 = rknpu_dev->genpd_dev_npu2;
	rknpu_dev->hw_res.multiple_domains = rknpu_dev->multiple_domains;
	rknpu_dev->hw_res.srsts = rknpu_dev->srsts;
	rknpu_dev->hw_res.num_srsts = rknpu_dev->num_srsts;
	rknpu_dev->hw_res.vdd = rknpu_dev->vdd;
	rknpu_dev->hw_res.mem = rknpu_dev->mem;
	rknpu_dev->hw_res.hw_powered = false;
...
static int rknpu_runtime_resume(struct device *dev)
{
	...
	/*
	 * The NPU power islands (PD_NPUTOP / PD_NPU1 / PD_NPU2) are
	 * managed manually by rknpu_power_on() / rknpu_power_off()
	 * via the genpd_dev_npu0..2 virtual devices, not by the
	 * device's own runtime-PM. Driver-core therefore force-resumes
	 * us on unbind (device_release_driver_internal() ->
	 * __pm_runtime_resume(dev)) even after rknpu_power_off() has
	 * already gated those islands. In that state any MMIO from
	 * rknpu_devfreq_runtime_resume() -> set_read_margin() -> regmap
	 * to NPU GRF will hit a powered-down bus and SError the kernel.
	 *
	 * rknpu_power_on() sets hw_powered = true after all islands and
	 * clocks are guaranteed up, and rknpu_power_off() clears it
	 * before tearing anything down. Use that as the gate.
	 */
	if (rknpu_dev && !rknpu_dev->hw_powered)
		return 0;
