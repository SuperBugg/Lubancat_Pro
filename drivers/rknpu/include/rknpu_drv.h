...
struct rknpu_device {
	...
	struct device *dev;
	struct rknpu_hw_resources hw_res;
	...
	bool multiple_domains;
	/*
	 * True while the NPU power islands (PD_NPUTOP / PD_NPU1 /
	 * PD_NPU2) and clocks are actually held up by rknpu_power_on().
	 * Used by the runtime-PM resume callback to detect the "driver
	 * core force-resumed us on unbind, but the islands are gated"
	 * case, where touching NPU MMIO would SError the kernel.
	 */
	bool hw_powered;
